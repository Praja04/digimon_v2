...
use App\Http\Requests\MonitoringStorageKimiaStoreRequest;
use App\Http\Requests\MonitoringStorageKimiaUpdateRequest;
use App\Http\Requests\ProductionBatchRequest;
use App\Models\MonitoringPasteurisasi;
use App\Models\MonitoringStorageKimia;
use App\Models\MonitoringStorageKimiaRelation;
...
class MonitoringStorageKimiaController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($monitoringStorageKimia)
                ...
                ->addColumn('detail', function ($data) {
                    $showUrl = route('monitoring-storage-kimia.show', ['id' => $data->id]);
        ...
        return view('app.monitoring_storage_kimia.index');
    }
    ...
    public function show($id)
    {
        $productionBatch = ProductionBatch::with([
            'MonitoringStorageKimia' => fn($query) => $query->with('additionalBatches'),
        ...
        foreach ($productionBatch->MonitoringStorageKimia as $mEntry) {
            ...
            $mRelated = DB::table('monitoring_storage_kimia_relations')
                ->where('monitoring_storage_kimia_id', $mEntry->id)
                ->pluck('batch')
                ->toArray();
        ...
        return view('app.monitoring_storage_kimia.show', compact(
            'productionBatch',
            'filteredBatchGroups'
        ));
    }
    ...
    {
        try {
            $exists = MonitoringStorageKimia::where('production_batch_id', $request->production_batch_id)
    ...
    public function getLastRevisi(Request $request)
    {
        ...
        $lastRevisi = MonitoringStorageKimia::where('production_batch_id', $request->production_batch_id)
            ->where('batch_range', $request->batch_range)
            ->max('revisi');
    ...
    public function getAvailableAdditionalBatch(Request $request)
    {
        ...
        $processCandidates = function (array $productionBatchIds) use ($validDispositions, $exclude) {
            $all = MonitoringPasteurisasi::whereIn('production_batch_id', $productionBatchIds)
                ->whereIn('disposition', $validDispositions)
                ->get();
            ...
            foreach ($grouped as $key => $items) {
                ...
                $related = DB::table('monitoring_pasteurisasi_relations')
                    ->where('monitoring_pasteurisasi_id', $chosen->id)
                    ->pluck('batch')
                    ->toArray();
                ...
                // jika batch sudah ada di monitoring_storage_kimia untuk PO yang sama, skip
                $monitoringEntries = MonitoringStorageKimia::where('production_batch_id', $chosen->production_batch_id)->get();
    ...
    public function getJalanBareng(Request $request)
    {
        ...
        $usedBatchIds = MonitoringStorageKimiaRelation::pluck('monitoring_storage_kimia_id')->toArray();

        // Ambil dari PO yang sama dulu
        $mainSame = MonitoringStorageKimia::where('production_batch_id', $productionBatchId)
        ...
        if ($mainSame->isNotEmpty()) {
            ...
            $mainBlending = MonitoringStorageKimia::where('production_batch_id', '!=', $productionBatchId)
                ->whereNotIn('disposition', $excludedDispositions)
                ->whereNotIn('id', $usedBatchIds)
                ->with('productionBatch')
                ->orderByDesc('id')
    ...
    public function storeRevisi(MonitoringStorageKimiaUpdateRequest $request)
    {
        try {
            $blending = MonitoringStorageKimia::findOrFail($request->id_revisi_blending);
            ...
            $exists = MonitoringStorageKimia::where('production_batch_id', $request->id_productbatch_revisi_blending)
            ...
            if ($disposition_revisi === 'Leveling') {
                ...
                $new = MonitoringStorageKimia::create($base + [
                ...
                foreach ($additionalBatches as $i => $batch) {
                    DB::table('monitoring_storage_kimia_relations')->insert([
                        'monitoring_storage_kimia_id' => $new->id,
                        'batch' => $batch,
                ...
                MonitoringStorageKimia::create($base + ['is_adjustment' => false]);
            }
