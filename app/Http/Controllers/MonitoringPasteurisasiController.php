...
use App\Http\Requests\MonitoringPasteurisasiStoreRequest;
use App\Http\Requests\MonitoringPasteurisasiUpdateRequest;
...
class MonitoringPasteurisasiController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($monitoringPasteurisasi)
                ...
                ->addColumn('detail', function ($data) {
                    $showUrl = route('monitoring-pasteurisasi.show', ['id' => $data->id]);
        ...
        return view('app.monitoring_pasteurisasi.index');
    }
    ...
    public function show($id)
    {
        ...
        foreach ($grouped as $batchRange => $items) {
            ...
            // related batches from monitoring_turun_blending
        ...
        return view('app.monitoring_pasteurisasi.show', compact(
            'productionBatch',
            'filteredBatchGroups'
        ));
    }

    public function store(MonitoringPasteurisasiStoreRequest $request)
    {
        try {
    ...
    public function getAvailableAdditionalBatch(Request $request)
    {
        ...
        $processCandidates = function (array $productionBatchIds) use ($validDispositions, $exclude) {
            ...
            foreach ($grouped as $key => $items) {
                ...
                // jika batch sudah ada di monitoring_pasteurisasi untuk PO yang sama, skip
    ...
    public function getJalanBareng(Request $request)
    {
        ...
        $mainSame = MonitoringPasteurisasi::where('production_batch_id', $productionBatchId)
    ...
    public function storeRevisi(MonitoringPasteurisasiUpdateRequest $request)
    {
        try {
            $blending = MonitoringPasteurisasi::findOrFail($request->id_revisi_blending);
            ...
            $exists = MonitoringPasteurisasi::where('production_batch_id', $request->id_productbatch_revisi_blending)
            ...
            if ($disposition_revisi === 'Leveling') {
                ...
                $new = MonitoringPasteurisasi::create($base + [
                ...
                MonitoringPasteurisasi::create($base + ['is_adjustment' => false]);
            }
