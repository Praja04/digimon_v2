...
class MonitoringPasteurisasiController extends Controller
{
    ...
    public function saveDraft(Request $request)
    {
        /*
        |--------------------------------------------------------------------------
        | Ambil data RAW + salinan untuk validasi
        |--------------------------------------------------------------------------
        |
        | RAW dipakai untuk disimpan ke tabel draft agar format input user
        | tetap persis, misalnya 0,8 tetap tersimpan sebagai "0,8".
        |
        | VALIDATION DATA hanya dipakai untuk proses validasi numeric.
        */
        $rawData = $request->all();
        ...
        foreach ($numericFields as $field) {
            if (
                array_key_exists($field, $validationData)
                && is_string($validationData[$field])
                && trim($validationData[$field]) !== ''
            ) {
        ...
        */
        ...
        try {
            ...
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | Simpan / Update Draft dari RAW DATA
            |--------------------------------------------------------------------------
            |
            | Satu monitoring_pasteurisasi_id hanya punya satu draft.
            | Data numeric disimpan sesuai persis input user.
            */
            $draft = MonitoringPasteurisasiDraft::updateOrCreate(
                ...
                [
                    ...
                    'nacl' => $rawData['nacl'] ?? null,
                    'bj' => $rawData['bj'] ?? null,
                    'visco' => $rawData['visco'] ?? null,
                    'aw' => $rawData['aw'] ?? null,
                    'buih' => $rawData['buih'] ?? null,
                    'ph' => $rawData['ph'] ?? null,
                    'organo' => $rawData['organo'] ?? null,
                    'endapan' => $rawData['endapan'] ?? null,
                    'aroma' => $rawData['aroma'] ?? null,
                    ...
                    'adjustment_qty_air' =>
                        $rawData['adjustment_qty_air'] ?? null,

                    'adjustment_qty_garam' =>
                        $rawData['adjustment_qty_garam'] ?? null,

                    'adjustment_qty_gula' =>
                        $rawData['adjustment_qty_gula'] ?? null,
