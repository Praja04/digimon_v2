<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('sampling_fisik_raw', function (Blueprint $table) {
            $table->id();
            $table->foreignId('id_identitas')->constrained('identitas_rm')->onDelete('cascade');
            $table->string('leleh')->nullable();
            $table->text('keterangan_leleh')->nullable();
            $table->string('warna')->nullable();
            $table->text('keterangan_warna')->nullable();
            $table->string('campuran')->nullable();
            $table->text('keterangan_campuran')->nullable();
            $table->string('aroma')->nullable();
            $table->text('keterangan_aroma')->nullable();
            $table->string('sesuai_std')->nullable();
            $table->text('keterangan_sesuai_std')->nullable();
            $table->foreignId('created_by')->nullable()->constrained('users')->onDelete('set null');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('sampling_fisik_raw');
    }
};
