                            ...
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="btn-group w-100" role="group">
                                        <input type="radio" class="btn-check" name="analisa_type" id="shortTerm" value="short-term">
                                        <label class="btn btn-outline-primary" for="shortTerm">Short Term</label>
                                        <input type="radio" class="btn-check" name="analisa_type" id="longTerm" value="long-term">
                                ...
                                <div class="col-md-6" id="jumlahWrapper">
                                    <input type="number" class="form-control" id="jumlahData" min="1" max="20" placeholder="Jumlah Sampel">
                                </div>
                        ...
                        <div id="analisaSection" style="display:none;">
                            ...
                            <div id="editJumlahWrapper" class="mb-3 p-3 bg-light rounded border" style="display:none;">
                                <div class="d-flex align-items-center gap-2 flex-wrap">
                                    ...
                                    <input type="number" class="form-control form-control-sm" id="editJumlahInput" min="1" max="20" style="max-width:100px;">
...
<style>
    ...
    #analisaAccordion .analisa-table tbody tr:last-child td {
        border-bottom: none;
    }
...
<script>
    ...
    // Short-term fields (no disposisi — handled separately below table)
    ...
    // Garam-Gula table fields (no disposisi — handled separately below table)
    ...
    function collectArrayValues() {
        const saved = {};
        $('#formAnalisa').find('input[type=text], input[type=number]').each(function() {
    ...
    function renderShortTermTable(jumlah) {
        ...
        for (let i = 1; i <= jumlah; i++) {
            ...
            SHORT_TERM_FIELDS.forEach(f => {
                const cls = f.type === 'decimal' ? 'decimal-input' : 'upper-input';
                tds += `
                        <td>
                            <input type="text"
        ...
        // Disposisi + Keterangan below table
        const disposisiHtml = `
    ...
    // ─────────────────────────────────────────────
    // FIELD BODY (non-short-term fields)
    // ─────────────────────────────────────────────
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <div id="disposisiAutoWrapper" style="display:none;">
                                    ...
                                    <input class="form-control form-control-sm bg-light" value="Release (otomatis)" readonly>
    ...
    function handleSubmit(e) {
        ...
        // For short-term & garam-gula: validate table cells
        if (currentType === 'short-term' || currentType === 'garam-gula') {
            let hasEmpty = false;
            ...
            $('#formAnalisa').find('.analisa-table tbody tr').each(function(rowIdx) {
                $(this).find('input[type=text]').each(function() {
                    if (!$(this).val().trim()) {
                        hasEmpty = true;
                        const name = $(this).attr('name');
                        // Find field label from the right fields array
                        const fieldDefs = currentType === 'garam-gula' ? GARAM_GULA_FIELDS : SHORT_TERM_FIELDS;
                        const field = fieldDefs.find(f => f.key === name);
            ...
            // Original validation for non-short-term
            ...
            $('#formAnalisa')
                ...
                .each(function() {
                    ...
                    if (label && !emptyInputs.includes(label)) emptyInputs.push(label);
            ...
            if (emptyInputs.length) {
                ...
                return Swal.fire({
                    ...
                    html: 'Field berikut masih kosong:<br><br>' +
                        emptyInputs.map(f => `<span class="badge bg-danger me-1 mb-1">${f}</span>`).join(''),
                    confirmButtonText: 'Oke',
                });
            }
